            ...
            <div class="slot-box-inner mb-3">
                <div class="slot-box-inner-heading d-flex justify-content-between align-items-center">
                    ...
                    @if($jobApplication->job_stage_id==$jobSchedule[0]->stage_id)
                        <a href="javascript:void(0)" class="btn btn-primary form-btn float-right batch-slot"
                           data-batch="{{ $batch }}">
                ...
                @foreach($jobSchedule as $key => $value)
                    <div
                        class="slot-data mb-3 {{$value->status == \App\Models\JobApplicationSchedule::STATUS_REJECTED ? 'slot-box-danger' : ''}}{{$value->status == \App\Models\JobApplicationSchedule::STATUS_SEND ? 'slot-box-success' : ''}}">
                        <div class="row p-3">
                            ...
                            <div class="form-group col-sm-6 mb-0">
                                <label name="notes"
                                       class="d-flex justify-content-between">{{ __('messages.job.notes').':' }}
                                    <div class="h-100" style="margin-top: -10px;">
                                        ...
                                        @endif
                                            <a title="{{ __('messages.common.delete') }}"
                                               class="btn btn-danger action-btn delete-btn h-100"
                                               data-id="{{$value->id}}"
                    ...
                    <div
                        class="row p-3 choose-slot-textarea {{ ($value->status == \App\Models\JobApplicationSchedule::STATUS_SEND) ? '' : 'd-none' }}">
