                    ...
                    <div class="col-12 col-sm-12 col-md-12 col-lg-12 candidate-experience"
                        ...
                        <article class="article article-style-b">
                            <div class="article-details">
                                ...
                                <span class="text-muted">{{ \Carbon\Carbon::parse($candidateExperience->start_date)->format('Y.m.d')}} - </span>

                                @if($candidateExperience->currently_working)
                                    <span class="text-muted">{{ __('messages.candidate_profile.present') }}</span>
                                @else
                                    <span class="text-muted"> {{\Carbon\Carbon::parse($candidateExperience->end_date)->format('Y.m.d')}} </span>
                                @endif
                                <span> | {{ $candidateExperience->country }}, {{ $candidateExperience->city }}</span>
                    ...
                    <div class="col-12 col-sm-12 col-md-12 col-lg-12 candidate-education"
                        ...
                        <article class="article article-style-b">
                            <div class="article-details">
                                ...
                                <span class="text-muted">{{ $candidateEducation->year }} - <?=empty($candidateEducation->year_to)?"Jelenleg is tart":$candidateEducation->year_to?> | {{ $candidateEducation->country }}, {{ $candidateEducation->city }}</span>
...
@push('page-scripts')
    <script>
        let addExperienceUrl = "{{ route('candidate.create-experience.admin') }}";
        let experienceUrl = "{{ url('admin/candidate-experience') }}/";
        let addEducationUrl = "{{ route('candidate.create-education.admin') }}";
        let candidateUrl = "{{ url('admin') }}/";
        let educationUrl = "{{ url('admin/candidate-education') }}/";
