...
class DashboardRepository
{
    ...
    public function getDashboardAssociatedData()
    {
        ...
        $data['totalCandidates'] = User::whereOwnerType(Candidate::class)->whereIsActive('1')->count();
        $data['totalEmployers'] = User::whereOwnerType(Company::class)->whereIsActive('1')->count();
    ...
    public function getEmployerDashboardData()
    {
        ...
        $data['pausedJobCount'] = Job::whereCompanyId($user->owner_id)->where('status', Job::STATUS_SUSPENDED)->count();
        $data['closedJobCount'] = Job::whereCompanyId($user->owner_id)->where('status', Job::STATUS_EXPIRED)->count();
        $data['unapprovedJobCount'] = Job::whereCompanyId($user->owner_id)->where('status', Job::STATUS_PENDING)->count();
        $data['jobCount'] = Job::whereCompanyId($user->owner_id)->where('status',
            Job::STATUS_ACTIVE)->whereDate('job_expiry_date', '>=', Carbon::now()->toDateString())->count();
        $data['expiringJobCount'] = Job::whereCompanyId($user->owner_id)->where('status',
            Job::STATUS_ACTIVE)->whereDate('job_expiry_date', '<=', Carbon::now()->addWeek()->toDateString())->count();
