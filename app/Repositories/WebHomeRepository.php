...
class WebHomeRepository
{
    ...
    {
        $data['candidates'] = Candidate::with('user')->whereHas('user', function (Builder $query) {
            $query->where('is_active', '=', true);
        })->count();
        $data['jobs'] = Job::status(Job::STATUS_ACTIVE)->count();
        $data['resumes'] = Media::where('model_type', Candidate::class)->where('collection_name',
            Candidate::RESUME_PATH)->count();
        $data['companies'] = []; /* Company::query()->whereHas('user', function (Builder $query) {
            $query->where('is_active', '=', true);
        })->count();*/
        ...
        /*$categories = JobCategory::whereIsFeatured(1)
            ->withCount([
                'jobs' => function (Builder $q) {
                    $q->where('status', '=', Job::STATUS_ACTIVE);
                },
            ])
...
//        ])->get();
        ...
        return Company::has('activeFeatured')
           ...
           /* ->whereHas('user', function (Builder $query) {
            ...
            ->withCount(['jobs' => function (Builder $q) {
                $q->where('status', '!=', Job::STATUS_DRAFT);
                $q->where('status', '!=', Job::STATUS_EXPIRED);
            }])
            ->orderBy('created_at', 'desc')
            ->get();
    }
    ...
    public function getRecentBlog()
    {
        return $blog = Post::with([
            'postAssignCategories', 'media', 'user' => function ($query) {
