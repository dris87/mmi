...
//        $query->whereHas('user', function (Builder $q) {
//            $q->when(! empty($this->searchByCity), function (Builder $query) {
//                $query->whereHas('city', function (Builder $user) {
//                    $user->where('name', 'like', '%'.strtolower($this->searchByCity).'%');
//                });
//                $query->orWhereHas('country', function (Builder $user) {
//                    $user->where('name', 'like', '%'.strtolower($this->searchByCity).'%');
//                });
//                $query->orWhereHas('state', function (Builder $user) {
//                    $user->where('name', 'like', '%'.strtolower($this->searchByCity).'%');
//                });
//            });
//        });
//        dd($query->toSql());
        ...
        $query->withCount([
            'jobs' => function (Builder $q) {
                ...
                $q->whereDate('job_expiry_date', '>=', Carbon::now()->toDateString());
            },
        ]);
