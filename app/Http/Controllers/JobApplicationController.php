...
class JobApplicationController extends AppBaseController
{
    ...
    {
        if ($slot->status == 1) {
            return $this->sendError('Assigned slot should not delete.');
        } else {
            $slot->delete();
    ...
    public function cancelSelectedSlot(Request $request)
    {
        ...
        $allJobSchedule = JobApplicationSchedule::where('stage_id', $lastRecord->stage_id)->where('batch',
            $lastRecord->batch)
            ->where('status', JobApplicationSchedule::STATUS_SEND)->get();
     ...
     * @return JsonResponse
     */
    public function checkStage($jobId): JsonResponse
    {
        $data = [];
        $jobApplication = JobApplication::where('job_id', $jobId)->first();
        $data['current_stage'] = $jobApplication->job_stage_id;
        $data['current_stage_cleared'] = JobApplicationSchedule::where('job_application_id',
            $jobApplication->id)->where('stage_id', $jobApplication->job_stage_id)
            ->where('status', JobApplicationSchedule::STATUS_SEND)->exists();
